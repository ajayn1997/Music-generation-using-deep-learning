...
def train(text, epochs=100, save_freq=10):
    ...
    idx_to_char = {i: ch for (i, ch) in enumerate(char_to_idx.items())}
    ...
    T = np.asarray([char_to_idx[c] for c in text], dtype=np.int32) # convert complete text into numerical data

    print('Length of the text' + str(T.size))
    ...
    for epoch in range(epochs):
        ...
        log.add_entry(np.mean(losses), np.mean(accs))
